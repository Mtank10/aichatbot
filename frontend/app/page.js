...
export default function Home() {
  ...
  const sendMessage = async () => {
    ...
    const response = await fetch(`${API_URL}/api/chat/${selectedChatId}/message`, {
      method: 'POST',
  ...
  const stopStreaming = async () => {
    if (abortController.current && selectedChatId) {
      await fetch(`${API_URL}/api/chat/${selectedChatId}/stop`, {
        method: 'POST',
      });
          ...
          <ul className="mt-2 space-y-2">
            {chats.map((chat) => (
              <li key={chat.id}>
               ...
               <span className="ml-2 text-sm"> {new Date(chat.created_at).toLocaleString('en-IN', {
          dateStyle: 'medium',
          timeStyle: 'short'
        })}</span>
        </div>
